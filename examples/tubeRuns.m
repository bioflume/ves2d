function Xfinal = tubeRuns(runName,VC,kappa,fmm)
...
prams.T = 3E+1;   % time horizon (or 100*lenScale/options.farFieldSpeed) etc.

options.timeAdap = true; % time adaptivity
options.nsdc = 0;  % number of sdc corrections
prams.areaLenTol = 1e-4; % tolerance for errors in area-length for time-stepping
...
options.logFile = ['output/tubeRuns/' runName '.log'];
% Name of binary data file for storing vesicle information
options.dataFile = ['output/tubeRuns/' runName 'Data.bin'];
...
X = [Xnew(1:end/2)-mean(Xnew(1:end/2));Xnew(end/2+1:end)-mean(Xnew(end/2+1:end))-0.3];
