...
%   traction = KT\([ext_force; ext_torque]);
  ...
  [~,uwallSL] = ker.exactStokesSL(bb, viscosity, traction, [], Xwalls, 1);
  [~,uwallDL] = ker.exactStokesDL(bb,K*U,[],Xwalls,1);
  uwall = uwallSL - uwallDL;
  ...
  title('Velocity on the body due to wall')
  axis equal
  ...
  uMag = mean(sqrt(ubody_found(1:end/2).^2 + ubody_found(end/2+1:end).^2));
  title('Velocity on the body after linear solve')
  disp(['Velocity mag on the body due to linear solve: ' num2str(uMag)]);
  pause
  end
