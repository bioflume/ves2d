function [ave_iter,max_iter,trajectories] = starSuspension_TestShravan(shearStrength, dt, N)
addpath ../src/
...
%   sol = TimeMatVec(rand(sys_size,1),bb, ker, SLP, DLP, DLPT, K, KT, Hmat, H, viscosity);
  if 1
  [sol,iflag,~,iter,~] = gmres(@(X) ...
      TimeMatVec(X,bb, ker, SLP, DLP, DLPT, K, KT, Hmat, H, viscosity),...
      RHS,[],1E-10,sys_size,[]);
  else
  [sol,iflag,~,iter,~] = gmres(@(X) ...
      TimeMatVec(X,bb, ker, SLP, DLP, DLPT, K, KT, Hmat, H, viscosity),...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function val = TimeMatVec(sol, bb, ker, SLP, DLP, DLPT, K, KT, H, Hvec, viscosity)
...
for k = 1 : nv
  SLvals(:,k) = 0.5*1./Hvec(:,k).*traction(:,k) + (SLP(:,:,k)-DLP(:,:,k)) * traction(:,k);
  KDLvals(:,k) = -K(:,:,k)*velocity(:,k);
...
DLinteract = ker.stokesDL_times_density(bb,bb,traction,1);
