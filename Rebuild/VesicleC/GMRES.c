...
{
	//These can't be declared at the global level
	//that's really bad if I need two levels of GMRES
	integer c__1 = 1;
	doublereal c_b7 = -1.;
	doublereal c_b8 = 1.;
	doublereal c_b20 = 0.;
...
/*        AV is temporary workspace here. */
	...
	dcopy_(n, &b[1], &c__1, &work[av * work_dim1 + 1], &c__1);
	(*matvec)(&c_b7, &x[1], &c_b8, &work[av * work_dim1 + 1], vMData);
    }
    (*psolve)(&work[r * work_dim1 + 1], &work[av * work_dim1 + 1], vPData);
...
L30:
    ...
    (*matvec)(&c_b8, &work[(v + i - 1) * work_dim1 + 1], &c_b20, &work[av * 
	    work_dim1 + 1], vMData);
    (*psolve)(&work[w * work_dim1 + 1], &work[av * work_dim1 + 1], vPData);
...
/*           tolerance, update the approximation vector X and quit. */
    ...
    *resid = (d__1 = work[i + 1 + s * work_dim1], abs(d__1)) / bnrm2;
	...
	update_(&i, n, &x[1], &h[h_offset], ldh, &work[y * work_dim1 + 1], &
		work[s * work_dim1 + 1], &work[v * work_dim1 + 1], ldw, &c__1, &c_b8);
	goto L70;
    }
    if (*iter == maxit) {
	goto L50;
...
/*     =============================================================== */
/* Subroutine */ int update_(i, n, x, h, ldh, y, s, v, ldv, c__1, c_b8)
integer *i, *n;
doublereal *x, *h;
integer *ldh;
doublereal *y, *s, *v;
...
/*     ========================================================= */
/* Subroutine */ int basis_(i, n, h, v, ldv, w, c__1)
integer *i, *n;
doublereal *h, *v;
integer *ldv;
doublereal *w;
