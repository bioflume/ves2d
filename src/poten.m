...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function o = poten(N,fmmPrecision,antiAlias)
...
if antiAlias
  if o.N <= 128
    o.LPuprate = ceil(sqrt(o.N));
  elseif o.N > 128 && o.N <= 1536
    o.LPuprate = 4;
  else
    o.LPuprate = 2;
  end
  [o.Restrict_LP,o.Prolong_LP] = fft1.fourierRandP(o.N,o.N*o.LPuprate);
else
  o.LPuprate = 1;
  o.Restrict_LP = [];
  o.Prolong_LP = [];
end
% build restriction and prolongation matricies
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [LP] = nearSingInt(o,vesicleSou,f,selfMat,selfMatNoCorr,...
...
% If the Nsou is too large, do not upsample to N^3/2, it is too expensive
% later. (e.g. if Nsou = 1000, N^3/2 = 31,622)
Nup = o.LPuprate*Nsou;
...
if tEqualS % sources == targets
  if nvSou > 1
    if (strfind(char(kernel),'fmm'))
      ...
      % this is used when fmm for DLP is in use
      farField = kernel(vesicleUp,fup,selfMatNoCorr);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [LP,nearField,farField] = divideNearFarSLP(o,...
        vesicleSou,f,selfMatNoCorr,NearStruct,kernel,kernelDirect,...
        vesicleTar,tEqualS)
% 
% vesicleSou : Source points
% f : density
...
vesicleUp = capsules(Xup,[],[],vesicleSou.kappa,...
  vesicleSou.viscCont,vesicleSou.antiAlias);
...
for k1 = 1:nvSou
  ...
  for k2 = K
    ...
    if (numel(J) ~= 0)
      ...
      % layer potential will be evaulted using Lagrange interpolant of
      % nearby points
      farField(J,k2) =  farField(J,k2) - ...
          potTar(1:numel(J));
      farField(J+Ntar,k2) =  farField(J+Ntar,k2) - ...
          potTar(numel(J)+1:end);
      
      
      nearField(J,k2) = nearField(J+Ntar,k2)+potTar(1:numel(J));
...
end % divideNearFarSLP
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function LP = evaluateLayerPoten(o,...
        vesicleSou,f,selfMatNoCorr,NearStruct,kernel,kernelDirect,...
        kernelRegul,vesicleTar,tEqualS)
% LP = evaluateLayerPoten(o,...
%        vesicleSou,f,selfMatNoCorr,NearStruct,kernel,kernelDirect,...
%        kernelRegul,vesicleTar,tEqualS)
% evaluates layer potential using regularized kernel for near-interactions
...
for k1 = 1:nvSou
  ...
  for k2 = K
    ...
    if (numel(J) ~= 0)
      ...
      [~,regulNear] = kernelRegul(vesicleUp,fup,[],[Xtar(J,k2);Xtar(J+Ntar,k2)],k1);
      nearField(J,k2) = nearField(J,k2)+regulNear(1:numel(J));
      nearField(J+Ntar,k2) = nearField(J+Ntar,k2)+regulNear(numel(J)+1:end);
...
if ~ialpert
  ...
  if vesicle.antiAlias
    Nup = o.LPuprate*vesicle.N;  
    Xup = [interpft(vesicle.X(1:end/2,:),Nup);...
            interpft(vesicle.X(end/2+1:end,:),Nup)];
    vesicleUp = capsules(Xup,[],[],vesicle.kappa,vesicle.viscCont,...
        vesicle.antiAlias);
    G = o.stokesSLmatrixAlpert(vesicleUp);
  else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function G = stokesSLmatrixKR(o,vesicle)
...
%   if vesicle.antiAlias
    G11 = o.Restrict_LP * G11 * o.Prolong_LP;
    G12 = o.Restrict_LP * G12 * o.Prolong_LP;
    G22 = o.Restrict_LP * G22 * o.Prolong_LP;
%   end
  
  G(:,:,k) = [G11 G12; G12 G22];
  % build matrix with 4 blocks
  G(:,:,k) = 1/4/pi*G(:,:,k)*2*pi/N; 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function G = stokesSLmatrixNoCorr(o,vesicle)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function G = stokesSLmatrixNoCorrRegul(o,vesicle)
% G = stokesSLmatrixNoCorr(vesicle) computes stokes single layer potential
% matrix without any quadrature. This corresponds to the way SLP FMM
% computes the SLP. Its counterpart is stokesDLmatrixNoCorr
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function D = stokesDLmatrixNoCorr(o,vesicle)
% D = stokesDLmatrixNoCorr(vesicle) builds matrix for computing self double
% layer potentials. However, it does not include the correction terms for
% the singularities unlike stokesDLmatrix. This matrix is used when we do
% FMM for V2V or W2W interactions. The reason is when we call FMM, it
% computes potentials including self-interaction. However,
% self-interactions must be removed b/c they are not correct. To remove
% them we need to compute self-interactions. So this matrix helps computing
% them efficiently.
...
for k=1:vesicle.nv  % Loop over curves
  if (vesicle.viscCont(k) ~= 1)
    ...
    % THIS IS THE DIFFERENCE, DO NOT ADD THE DIAGONAL LIMITING TERM
    % D11(1:N+1:N.^2) = 0.5*const_coeff*cur.*sa(1,:).*txsou.^2;
    % diagonal limiting term
...
S1 = zeros(2*vesicleOrg.N,2*vesicleOrg.N,vesicle.nv);
S2 = zeros(2*vesicleOrg.N,2*vesicleOrg.N,vesicle.nv);
% Whole matrix
...
for k=1:vesicle.nv  % Loop over curves
  S1k = zeros(2*N,2*N);
  S2k = zeros(2*N,2*N);
  % initialize double-layer potential to zero for each matrix
  index1 = (1:2:N)'; % odd-indexed source points
  for j=2:2:N % Loop over targets
    ...
    if vesicle.antiAlias
      xx = interpft(xx,N); yy = interpft(yy,N);
      vesicle = capsules([xx;yy],[],[],vesicle.kappa,vesicle.viscCont(k),true);
      % upsampled single vesicle        
    end
    
    rho4 = ((xx(j) - xx(index1)).^2 + (yy(j) - yy(index1)).^2).^2;
    % distance squared

    coeff = (xx(j) - xx(index1)).^3.*vesicle.sa(index1)./rho4/pi;
    S1k(j,index1) = 4*pi/vesicle.N*coeff;
    % part of first component of stress1 that multiplies first 
    % component of traction jump

    coeff = (xx(j) - xx(index1)).^2.*(yy(j) - yy(index1)).* ...
        vesicle.sa(index1)./rho4/pi;
    S1k(j,index1+vesicle.N) = 4*pi/vesicle.N*coeff;
    % part of first component of stress1 that multiplies second
    % component of traction jump
    S1k(j+vesicle.N,index1) = 4*pi/vesicle.N*coeff;
    % part of second component of stress1 that multiplies first 
    % component of traction jump
    S2k(j,index1) = 4*pi/vesicle.N*coeff;
    % part of first component of stress2 that multiplies first 
    % component of traction jump

    coeff = (xx(j) - xx(index1)).*(yy(j) - yy(index1)).^2.* ...
        vesicle.sa(index1)./rho4/pi;
    S1k(j+vesicle.N,index1+vesicle.N) = 4*pi/vesicle.N*coeff;
    % part of second component of stress1 that multiplies second 
    % component of traction jump
    S2k(j,index1+vesicle.N) = 4*pi/vesicle.N*coeff;
    % part of first component of stress2 that multiplies second 
    % component of traction jump
    S2k(j+vesicle.N,index1) = 4*pi/vesicle.N*coeff;
    % part of second component of stress2 that multiplies first 
    % component of traction jump

    coeff = (yy(j) - yy(index1)).^3.*vesicle.sa(index1)./rho4/pi;
    S2k(j+vesicle.N,index1+vesicle.N) = 4*pi/vesicle.N*coeff;
  ...
  if vesicle.antiAlias
    S11k = o.Restrict_LP * S1k(1:N,1:N) * o.Prolong_LP;
    S12k = o.Restrict_LP * S1k(1:N,N+1:2*N) * o.Prolong_LP;
    S21k = o.Restrict_LP * S1k(N+1:2*N,1:N) * o.Prolong_LP;
    S22k = o.Restrict_LP * S1k(N+1:2*N,N+1:2*N) * o.Prolong_LP;
    
    S1(:,:,k) = [S11k S12k; S21k S22k];
    
    S11k = o.Restrict_LP * S2k(1:N,1:N) * o.Prolong_LP;
    S12k = o.Restrict_LP * S2k(1:N,N+1:2*N) * o.Prolong_LP;
    S21k = o.Restrict_LP * S2k(N+1:2*N,1:N) * o.Prolong_LP;
    S22k = o.Restrict_LP * S2k(N+1:2*N,N+1:2*N) * o.Prolong_LP;
    
    S2(:,:,k) = [S11k S12k; S21k S22k];
  else
    S1(:,:,k) = S1k; 
    S2(:,:,k) = S2k;
  end
...
% TODO: Mimi like exactStokesSLdiag
...
function [stokesSLP,stokesSLPtar] = ...
    exactStokesSL(o,vesicle,f,M,Xtar,K1)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [stokesSLP,stokesSLPtar] = ...
    exactStokesSLregul(o,vesicle,f,M,Xtar,K1)
% [stokesSLP,stokesSLPtar] = exactStokesSL(vesicle,f,Xtar,K1) computes
% the single-layer potential due to f around all vesicles except
% itself.  Also can pass a set of target points Xtar and a collection
% of vesicles K1 and the single-layer potential due to vesicles in K1
...
h = max(vesicle.length/vesicle.N);
% arc-length spacing
...
for k = 1:ncol % loop over columns of target points 
  ...
  coeff = 0.5*log(dis2).*erf(2.5*sqrt(dis2)/h);
  % first part of single-layer potential for Stokes
  stokesSLPtar(1:Ntar,k) = -sum(coeff.*denx,2);
  stokesSLPtar(Ntar+1:2*Ntar,k) = -sum(coeff.*deny,2);
  % log part of stokes single-layer potential

  coeff = erf(2.5*sqrt(dis2)/h).*(diffx.*denx + diffy.*deny)./dis2;
  % second part of single-layer potential for Stokes
  stokesSLPtar(1:Ntar,k) = stokesSLPtar(1:Ntar,k) + ...
      sum(coeff.*diffx,2);
  stokesSLPtar(Ntar+1:2*Ntar,k) = stokesSLPtar(Ntar+1:2*Ntar,k) + ...
...
end % exactStokesSLregul
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [stokesSLP,stokesSLPtar] = ...
    exactStokesSLold(o,vesicle,f,Xtar,K1)
% [stokesSLP,stokesSLPtar] = exactStokesSL(vesicle,f,Xtar,K1) computes
% the single-layer potential due to f around all vesicles except
% itself.  Also can pass a set of target points Xtar and a collection
% of vesicles K1 and the single-layer potential due to vesicles in K1
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [stokesDLP,stokesDLPtar] = ...
    exactStokesDLregul(o,vesicle,f,M,Xtar,K1)
% [stokesDLP,stokesDLPtar] = exactStokesDL(vesicle,f,Xtar,K1) computes
% the double-layer potential due to f around all vesicles except
% itself.  Also can pass a set of target points Xtar and a collection
% of vesicles K1 and the double-layer potential due to vesicles in K1
...
for k = 1:ncol % loop over columns of target points
  ...
  rdotnTIMESrdotf = (diffx.*normalx + diffy.*normaly)./dis2.^2 .* ...
      (diffx.*denx + diffy.*deny).*erf(2.5*sqrt(dis2)/h);
  % \frac{(r \dot n)(r \dot density)}{\rho^{4}} term
...
if (nargin == 4 && vesicle.nv > 1)
  ...
  for k = 1:vesicle.nv
    ...
    for j=1:vesicle.N
      ...
      rdotfTIMESrdotn = erf(2.5*sqrt(dis2)/h).* ...
        (diffxy(1:vesicle.N,:).*nx + ...
        diffxy(vesicle.N+1:2*vesicle.N,:).*ny)./dis2.^2 .* ...
        (diffxy(1:vesicle.N,:).*denx + ...
        diffxy(vesicle.N+1:2*vesicle.N,:).*deny);
...
end % exactStokesDLregul
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [stokesDLP,stokesDLPtar] = ...
    exactStokesDLold(o,vesicle,f,Xtar,K1)
% [stokesDLP,stokesDLPtar] = exactStokesDL(vesicle,f,Xtar,K1) computes
% the double-layer potential due to f around all vesicles except
% itself.  Also can pass a set of target points Xtar and a collection
% of vesicles K1 and the double-layer potential due to vesicles in K1
...
end % exactStokesDLold

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [stokesSLP,stokesSLPtar] = ...
    exactStokesSLfmmOld(o,vesicle,f,Xtar,K)
...
if (nargin == 5)
  ...
  % Subtract potential due to each vesicle on its own.  Nothing to
...
end % exactStokesSLfmmOld

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [stokesSLP,stokesSLPtar] = ...
    exactStokesSLfmm(o,vesicle,f,SLPmat,Xtar,K)
% [stokesSLP,stokeSLPtar] = exactStokesSLfmm(vesicle,f,Xtar,K) uses the
% FMM to compute the single-layer potential due to all vesicles except
% itself vesicle is a class of object capsules and f is the density
% function NOT scaled by arclength term.  Xtar is a set of points where
...
if (nargin == 6)
  ...
  SLPself = o.exactStokesSLdiag(vesicle,SLPmat,f);
  ...
  %  stokesSLP(:,k) = stokesSLP(:,k) - [u;v];
  %end
  %disp('Computing V2V SLP')
  %err = norm(stokesSLP2-stokesSLP)/norm(stokesSLP)
  %pause
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [stokesDLP,stokesDLPtar] = ...
    exactStokesDLnewfmm(o,vesicle,f,DLPmat,Xtar,K)
...
if (nargin == 6)
  ...
  % we need to remove the self-interaction terms, so compute those terms
  % using the self-interaction matrix
  DLPself = o.exactStokesDLdiag(vesicle,DLPmat,f);
  stokesDLP = stokesDLP-DLPself;
  
  % Below is not an efficient way of subtracting the self-interaction terms
  %for k = 1:vesicle.nv
...
function [laplaceSLP,laplaceSLPtar] = ...
    exactLaplaceSL(o,vesicle,f,M,Xtar,K1)
...
function [laplaceDLP,laplaceDLPtar] = ...
    exactLaplaceDL(o,vesicle,f,M,Xtar,K1)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [laplaceDLP,laplaceDLPtar] = ...
    exactLaplaceDLold(o,vesicle,f,Xtar,K1)
% pot = exactLaplaceDL(vesicle,f,Xtar,K1) computes the double-layer
% laplace potential due to f around all vesicles except itself.  Also
% can pass a set of target points Xtar and a collection of vesicles K1
% and the double-layer potential due to vesicles in K1 will be
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [laplaceDLP,laplaceDLPtar] = ...
    exactLaplaceDLfmm(o,vesicle,f,M,Xtar,K)
...
if nargin == 5
  ...
  ncol = 0;
  Ntar = 0;
  % if nargin ~= 5, user does not need the layer potential at arbitrary points
end
...
[stress1,stress2] = vesicle.stressTensor(f,[],pressTar,fmm,'SLP',o,[]);
