...
%   build reference elliptical vesicle
  ...
  if any(strcmp(options,'volFrac'))
    ...
    ttObj = options{ind+3};
    X = oc.fillDomain(X0s(:,1),'domain','couette','volFrac',volFrac,'walls',Xwalls,'tstepObj',ttObj);
    nv = size(X,2);
  end
...
if isempty(X)
  ...
  X = zeros(2*N,nv);
  for k=1:nv
    X(1:N,k) = cos(theta(k)) * X0s(1:N,k) - ...
      sin(theta(k)) * X0s(N+1:2*N,k) + cen(1,k);
    X(N+1:2*N,k) = sin(theta(k)) * X0s(1:N,k) +  ...
      cos(theta(k)) * X0s(N+1:2*N,k) + cen(2,k);
  end
