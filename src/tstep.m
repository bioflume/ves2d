...
o.farField = @(X,Xint) o.bgFlow(X,options.farField,...
    'Speed',options.farFieldSpeed,'Umax',options.farFieldUmax,...
    'Gamma',options.farFieldCurvature,'vortexSize',prams.vortexSize,...
...
elseif any(strcmp(varargin,'parabolic2'))
  Gamma = find(strcmp(varargin,'Gamma'));
  if isempty(Gamma)
      Gamma = 1;
  else
    Gamma = varargin{Gamma+1};
  end
  UM = find(strcmp(varargin,'Umax'));
  if isempty(UM)
    UM = 10;
