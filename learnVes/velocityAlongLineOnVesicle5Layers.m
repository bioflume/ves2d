...
nlayers = 5;
dlayer = [0; 1/2; 1; -1/2; -1] * maxLayerDist;
...
velNearLayer = op128.nearSingInt(vesicle,bendF,SLP,[],NearV2Tlayers,kernel,kernelDirect,traLayers,false,false); 
...
opX = rbfcreate(Xin,velXInput,'RBFFunction','gaussian');
opY = rbfcreate(Xin,velYInput,'RBFFunction','gaussian');
...
kernel = @op32.exactStokesSL;
kernelDirect = @op32.exactStokesSL;
SLP = @(X) op32.exactStokesSLdiag(vesicleLow,G32,X);

Xlayers = [xlayers32(:,4);xlayers32(:,5);ylayers32(:,4);ylayers32(:,5)];
traLayers.N = numel(Xlayers)/2;
traLayers.nv = 1;
traLayers.X = Xlayers;
[~,NearV2Tlayers] = vesicleLow.getZone(traLayers,2);

velNearLayer = op32.nearSingInt(vesicleLow,bendF32,SLP,[],NearV2Tlayers,kernel,kernelDirect,traLayers,false,false); 
velx32(:,4:5) = reshape(velNearLayer(1:end/2),32,2);
vely32(:,4:5) = reshape(velNearLayer(end/2+1:end),32,2);
...
% hold on
plot(d2ves/h,velTraMLARMRBF/max(abs(velTraNear)),'Color',[230 97 1]/255,'linewidth',2)
% plot(d2ves/h,velTraMLARMRBF32/max(abs(velTraNear)),'--','Color',[69,117,180]/255,'linewidth',2)
...
grid on
box on
figName = ['~/Desktop/IdP' num2str(idP) '_5Layers.png'];
ax = gca;
...
% 
figure(2); 
plot(d2ves/h,velTraMLARMRBF32/max(abs(velTraNear)),'Color',[230 97 1]/255,'linewidth',2)
axis square
xlim([-2 2])
ylim([0.8 1.1])
xticks([-2, -1, 0, 1, 2])
...
grid on
box on
figName = ['~/Desktop/IdP' num2str(idP) '_N32_5Layers.png'];
ax = gca;
exportgraphics(ax,figName,'Resolution',300)
