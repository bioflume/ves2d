...
DpartMicron = 4.7;
Dparticle = DpartMicron*DLDscale; % micron * scale [1.8, 4.7, 6.5, 7.8, 9.8]
volFrac = 0.20; %5%, 10%, 20%, 30%, 40%
RAell = 0.95;

% Parameters
runName = ['validRun_VF' num2str(volFrac*100) '_Dpart' num2str(10*DpartMicron) '_sample3'];
putBackLimit = 12; % 6 is 1 period for period = 15
...
% DLD parameters
Dpost = 20 * DLDscale; % micron * scale
Dx = 16 * DLDscale;
Dy = Dx;
...
period = 12; % must be 15
theta = atan(1/period);
Ufar = 120; % Ca should be 4(474), but this is large for high VF to be stable
% Ca = Reff^3 * Ufar / Dy/2
if volFrac >= 0.20 
  Ufar = 80;
end
...
VCs = 1;
kappaRigid = 1;
prams.vesViscCont = 50;

%if DpartMicron > 1.8 && DpartMicron < 6.5
%  prams.vesViscCont = 25;
%elseif DpartMicron >= 6.5
%  prams.vesViscCont = 10;
%else
%  prams.vesViscCont = 50; % viscosity contrast of the 1st type vesicle
%end
...
% Build poten operator for rigid body which has higher resolution than
% vesicles
dnn.Nrigid = Nrigid;
dnn.opRigid = poten(Nrigid);
...
% -------------------------------------------------------------------------
if irepulsion
  vesicle = capsules(X(:,1),[],[],prams.kappa,prams.viscCont(1),true);
  vesicle.setUpRate();
  dnn.minDist = 0.4;
...
tt.dt = 1E-4; sig = zeros(prams.N,prams.nv); 
...
for iter = 1 : 2
  ...
  [X,sig,etaInt,etaExt,RS] = tt.timeStepSimpleDiffDisc(X,sig,etaInt,etaExt,...
      RS,prams.viscCont,dnn.wallsInt,dnn.wallsExt,vesicle);
end
tt.dt = prams.dt;
% ------------------------------------------------------------------------
...
it = 1;
save(fileName,'it','XhistStore','Xfrozen','XwallsInt','XwallsExt',...
    'rem_ids','frozen_ids','time','Dx','Dy','Dpostx',...
    'Dposty','Ufar','volFrac','nPutBacks','nputLater','nZigZags',...
...
for it = 2 : ntime
  ...
  [Xhist,sigStore,etaInt,etaExt,RS] = dnn.DNNsolveMixed_wRigidDLP(...
      Xhist,sigStore,etaInt,etaExt,RS,Nnet,prams.viscCont);
  ...
  Xhist(:,2:end) = oc.alignCenterAngle(Xhist(:,2:end),Xiter);
    
  % Reparameterize rigid particle
  Xiter1 = Xhist(:,1);
  [Xiter1, ~] = oc.reparametrize(Xiter1,[],6,20);
  Xhist(:,1) = oc.alignCenterAngle(Xhist(:,1),Xiter1);
  ...
  [Xhist,sigStore,Xfrozen,rem_ids,frozen_ids,prams,nPutBacks,...
      ...
      nZigZags,initCys,finalCys,zzID,newID,tt,dnn.wallsInt, dnn.wallsExt,init_nv);

  prams.nv = numel(rem_ids);
  % First save current step,
  XhistStore{end+1} = Xhist;
  
  if rem(it,isave) == 0
    save(fileName,'it','XhistStore','Xfrozen','initCys','finalCys','iPartZZ',...
  ...
  if iPartZZ
    disp('RA = 0.9 zig-zagged. COMPLETED!')
    save(fileName,'it','XhistStore','Xfrozen','initCys','finalCys','iPartZZ',...
        'rem_ids','frozen_ids','nPutBacks','nputLater','nZigZags','zzID','newID','-append')
    break;
  end
...
function [Xhist,sigStore,Xfrozen,rem_ids,frozen_ids,prams,nPutBacks,...
    ...
    rem_ids,frozen_ids,nPutBacks,area0,len0,nputLater,nZigZags,initCys,finalCys,zzID,newID,tt,wallsInt,wallsExt,init_nv)
...
[~,~,lenscale] = oc.geomProp(X(:,2)); lenscale = max(lenscale);
arcLenSpacing = lenscale / prams.N; % arc-length spacing
...
% Find if there is any vesicle is nearly-colliding with the rigid body
%vesicleX0 = capsules(X0,[],[],[],[],1);
%vesicleX0.setUpRate();
%NearV2VX0 = vesicleX0.getZone([],1);
%[icollisionVes,collidingIdcs,safeIdcs] = vesicleX0.collisionWVesOutput(NearV2VX0,tt.fmm,tt.op);
% then freeze them
%will_freeze = [will_freeze; collidingIdcs(collidingIdcs~=1)];
%nputLater = nputLater + numel(collidingIdcs(collidingIdcs~=1));

pillarGrad = prams.epsilon*(prams.Dposty+prams.Dy)/(prams.Dpostx+prams.Dx);

% 1) MOVE PARTICLE (RA = 0.9) BACK IF IT REACHES TO THE END OF THE DEVICE
...
if centxn>=prams.xfreeze
  ...
  vesIDsInlet = find(sqrt((centyS-centyNew).^2 + (centxS-centxNew).^2) <= lenscale);
  % if there is then freeze them
  will_freeze = [will_freeze; vesIDsInlet];
  %nputLater = nputLater + numel(vesIDsInlet);
  
  X(:,1) = [X(1:end/2,1)-centxn+centxNew;X(end/2+1:end,1)-centyn+centyNew];
  Xlarge(:,1) = [Xlarge(1:end/2,1)-centxn+centxNew;Xlarge(end/2+1:end,1)-centyn+centyNew]; 
  ...
  nearZoneIDs = K(sqrt((centxn-centxK).^2+(centyn-centyK).^2) <= lenscale);
  ...
  for k2 = nearZoneIDs
    ...
    if numel(J)~=0
      ...
      if min(X(1:end/2,k2)-centxn+centxNew)>prams.xrange
        ...
        Xlarge(:,k2) = [Xlarge(1:end/2,k2)-centxn+centxNew; Xlarge(end/2+1:end,k2)-centyn+centyNew];
        ...
        nearZonek2IDs = K2(sqrt((centxn-centxK2).^2+(centyn-centyK2).^2) <= lenscale);
        
        for k3 = nearZonek2IDs
          J = find(zoneV2V{k3}(:,k2) == 1);
          if numel(J)~=0
            centyn = mean(interpft(X(end/2+1:end,k3),256));
            centxn = mean(interpft(X(1:end/2,k3),256));  
            centxNew = centxn-4*(prams.Dx+prams.Dpostx);
            centyNew = centyn-pillarGrad*4*(prams.Dx+prams.Dpostx);
            if min(X(1:end/2,k3)-centxn+centxNew)>prams.xrange
              X(:,k3) = [X(1:end/2,k3)-centxn+centxNew;X(end/2+1:end,k3)-centyn+centyNew];
              Xlarge(:,k3) = [(Xlarge(1:end/2,k3)-centxn)+centxNew;(Xlarge(end/2+1:end,k3)-centyn)+centyNew];
            else
...
% 2) MOVE OTHER VESICLES BACK IF THEY REACH TO THE END OF THE DEVICE
for k = 2 : prams.nv
  ...
  if centxn>=prams.xfreeze
    ...
    centxNew = centxn-4*(prams.Dx+prams.Dpostx);
    centyNew = centyn-pillarGrad*4*(prams.Dx+prams.Dpostx);
    Xprov = [Xlarge(1:end/2,k)-centxn+centxNew; Xlarge(end/2+1:end,k)-centyn+centyNew];
    vesicleTest = capsules([Xlarge(:,1) Xlarge(:,K) Xprov], [], [], [], [], true);
    vesicleTest.setUpRate();
    NearV2V = vesicleTest.getZone([],1);
    [icollisionVes,~] = vesicleTest.collision([],NearV2V,[],tt.fmm,tt.op);
    ...
    if ~icollisionVes && min(X(1:end/2,k)-centxn+centxNew) > prams.xrange
      X(:,k) = [X(1:end/2,k)-centxn+centxNew;X(end/2+1:end,k)-centyn+centyNew];
      Xlarge(:,k) = [(Xlarge(1:end/2,k)-centxn)+centxNew;(Xlarge(end/2+1:end,k)-centyn)+centyNew];
      % check if there is any vesicle in near-zone of k, if so, move it back,
      % too
      ...
      %    if min(X(1:end/2,k2)-centxn+centxNew)>prams.xrange
      %      X(:,k2) = [X(1:end/2,k2)-centxn+centxNew;X(end/2+1:end,k2)-centyn+centyNew];
      %      Xlarge(:,k2) = [1.1*(X(1:end/2,k2)-centxn)+centxNew;1.1*(X(end/2+1:end,k2)-centyn)+centyNew];
      %    else
      %      nputLater = nputLater + 1;
      %      will_freeze = [will_freeze; k2];
      %    end
...
% 3) DECIDE WHICH VESICLES ZIG-ZAGGED AND FREEZE ALL TO BE FROZEN
% Loop and check which ones zig-zagged and need to be frozen
delLat = (prams.Dy+prams.Dposty)*prams.epsilon;

iPartZZ = false;
if mean(X(end/2+1:end,1)) <= -1.5*prams.Dy-2*prams.Dposty+4*delLat
...
for iv = 2:prams.nv
  centx = mean(X(1:end/2,iv));
  % find the point that is closest to the vesicle below
  ids = find(abs(centx-wallsExt.X(1:end/2))<=1e-2 & wallsExt.X(end/2+1:end)<0);
  
  % Distance in y direction to the wall
  dist2wall = abs(min(X(end/2+1:end,iv))-wallsExt.X(ids+wallsExt.N));
  
  if dist2wall <= 4*arcLenSpacing % if distance is closer than 4*arc length spacing, then it zig-zagged
    % tag the ones we need to freeze
...
nputLater = init_nv - prams.nv;

% 4) FINALLY, ADD NEW VESICLES TO MAINTAIN THE VOLUME FRACTION
idOfLast = max([rem_ids;frozen_ids]); % id of last vesicle

% regions where a new vesicle can be introduced
xrange = prams.xrange + [prams.Dx/2 1.25*prams.Dx+prams.Dpostx];
yranges(1,:) = prams.Dy/2+prams.Dposty+[0.05*prams.Dy 0.95*prams.Dy]; % top lane
...
while (countPutLater<nputLater && doItOneMore)
  ...
  vesPlaced = false; iterLane = 1;
  while (~vesPlaced && iterLane <= 3)
    iter = 0;
    while (~vesPlaced && iter <= 10)
      iter = iter + 1;
      centxNew = xrange(1) + (xrange(2) - xrange(1))*rand;
      lane = whichLane(iterLane);
      centyNew = yranges(lane,1) + (yranges(lane,2)-yranges(lane,1))*rand;
      ...
      [icollisionVes,icollisionWallInt] = vesicle.collision(wallsInt,NearV2V,NearV2WInt,tt.fmm,tt.op);
      
      if ~icollisionWallInt && ~icollisionVes
        vesPlaced = true; 
        ...
        sigStore(:,end+1) = sigStore(:,refVesId);
...
function [X,area0,len0,prams] = initializeVesicles(Xref,prams,...
    XwallsInt,XwallsExt,tt,oc,RAell,Dparticle)
...
Xell = Xell/(max(Xell(end/2+1:end))-min(Xell(end/2+1:end)))*Dparticle;
