import torch
from pdeNet_Ves_fat_factor_modein5_onelevel import pdeNet_Ves_fat_factor_modein5_onelevel # from file import model class
import numpy as np
...
theta = np.arange(32)/32*2*np.pi
theta = theta.reshape(32,1)
bases = 1/32*np.exp(1j*theta*np.arange(32).reshape(1,32))  
...
model = pdeNet_Ves_fat_factor_modein5_onelevel(12, 2.0, 26)

output_list = []
for ij in np.arange(0,nmodes-1):
  imode = mod_list[ij+1]
  imode_net = "/Users/gokberk/Documents/GitHub/ves2d/learnVes/shannets/adv_fft_ds32/ves_adv_downsample_fft_2024Oct_mode" + str(imode) + ".pth"
  model.load_state_dict(torch.load(imode_net, map_location="cpu"))
  model.eval()

  # Get the fourier mode corresponding to imode
  basis = rr_ii_Mat[imode-1,:].reshape(1,1,64).float()
  ...
  input_net = torch.zeros(num_ves,2,64)
  for k in np.arange(0,num_ves):
      input_net[k,0,:32] = input_vec[k,0,:]
      input_net[k,0,32:] = input_vec[k,1,:]
      input_net[k,1,:] = basis
