...
for j = 1 : nParEvals
  ...
  %submitDLDrunManyVes_Explicit(flgSmooth(j),flgIntersect(j),...
...
for j = 1 : nParEvals
  ...
  if ~flgIntersect(j) && flgSmooth(j) && flgSpacing(j)
    ...
    if exist([o.saveFolder runNames{j} '.mat'],'file') == 2
      disp(['File for the run ' num2str(j) ' exists'])
...
%           (Yend-initCys(1)+(Dpostx+Dx)*tan(o.theta))/Dy;
      ...
      if nZigZags == 0 % simulation crashes
        Fvals(j) = 1E+4;
      elseif ~iPartZZ
        if abs(mean(Xhist(1:end/2,1))-mean(XwallsInt(1:end/2,1)))>Dx ...
