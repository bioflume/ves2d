...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Xnew,tenNew,etaIntNew,etaExtNew,RSnew] = DNNsolve(o,Xold,...
...
if o.repulsion
  repulsion = vesicle.repulsionScheme(Xold,o.repStrength,o.minDist,...
        [],wallsInt,wallsExt);
  [~,nearField,Frepulsion] = op.divideNearFarSLP(vesicle,...
        repulsion,SLPnoCorr,NearV2V,kernel,kernelDirect,vesicle,true);      
  if o.useNear
    Frepulsion = nearField + Frepulsion; 
  end
  [~,nearField,FREPwallInt] = op.divideNearFarSLP(vesicle,repulsion,SLP,[],...
      NearV2Wint,kernel,kernelDirect,wallsInt,false);
  if o.useNear
    FREPwallInt = FREPwallInt + nearField;
  end
  [~,nearField,FREPwallExt] = op.divideNearFarSLP(vesicle,repulsion,SLP,[],...
      NearV2Wext,kernel,kernelDirect,wallsExt,false);
  if o.useNear
    FREPwallExt = FREPwallExt + nearField;
  end
else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Xnew,tenNew,etaIntNew,etaExtNew,RSnew] = DNNsolveMixed(o,Xold,...
        tenOld,etaIntOld,etaExtOld,RSold,Nnet)
