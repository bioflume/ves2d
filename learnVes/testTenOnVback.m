...
vBack = @(X) 100*[X(end/2+1:end); -X(1:end/2)]...
...
MVinfTrue = zeros(N,nRandVes);
MVinfApp = zeros(N,nRandVes);
...
for ives = 1 : nRandVes
  ...
  Xinput = prepareInputForNet(Xstand,'tensionOnFourier',scale_MV,muChan_MV,...
      sdevChan_MV,offset_MV,colMeans,evects,nComp,[]);
  % Approximate the multiplication M*(FFTBasis)     
  Z1 = zeros(Nnet,numel(velActiveModes)); Z2 = Z1;

  for k = 1 : numel(velActiveModes)
    pred = predict(MVnets{k},Xinput)';
    Z1(:,k) = interpft(pred(1:MVoutSize/2),Nnet);
    Z2(:,k) = interpft(pred(MVoutSize/2+1:MVoutSize),Nnet);
  end  
  ...
  MVextStand = (Z1*V1+Z2*V2);
  
  % Destandardize the multiplication
  MVext = zeros(size(MVextStand));
  MVext(sortIdx) = MVextStand;

  MVinfApp(:,ives) = interpft(MVext,N);
  ...
  err(ives) = sqrt(sum((MVinfTrue(:,ives)-MVinfApp(:,ives)).^2))./...
