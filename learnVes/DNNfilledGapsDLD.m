...
prams.folderName = './output/VF20Ca1p5_circPost/';
...
for it = 2 : ntime
  ...
  save(fileName,'it','Xhist','Xfrozen','sigStore','ncountDNN','ncountExct')
  ...
  if max(nPutBacks) == prams.periodN
    disp('at least one vesicle has been put back # of period times. COMPLETED!')    
  end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k = 1 : nv
  ...
  if centxn>=prams.xfreeze
    ...
    centxNew = centxn-(ceil(prams.periodN)+1)*(prams.Dx+prams.Dpostx);
    centyNew = centyn-pillarGrad*(ceil(prams.periodN)+1)*(prams.Dx+prams.Dpostx);
...
xrange(2) = prams.xfreeze-prams.Dpostx-prams.Dx; % right point
