...
% for it = 1 : 1
  ...
  tracJumpTrueForXsolve = fBend + fTenTrue;
  % 
  ...
  SLP = @(X) op.exactStokesSLdiag(vesicle,G,X);
  farFieldtracJumpTrue = op.nearSingInt(vesicle,tracJumpTrueForXsolve,SLP,SLPnoCorr,NearV2V,...
      kernel,kernelDirect,vesicle,true,false);
  ...
  fname = ['~/Desktop/randFromGT/comparisonX' num2str(iter) '.mat'];
  save(fname,'X', 'tracJumpBeforeTenSolve' , 'farFieldtracJumpForTenSolve', 'tension', 'trueVbackSolve','trueSelfBendSolve','vback','tracJumpTrueForXsolve','farFieldtracJumpForXsolve', 'XadvTrue', 'XnewTrue')
