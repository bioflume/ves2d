...
Xics = [interpft(Xinput(1:end/2,1:21),32);interpft(Xinput(end/2+1:end,1:21),32)];
Xics128 = Xinput(:,1:21);
...
% for it = 1 : 1
  ...
  vesicle128 = capsules(X128,[],[],1,1,0);
  ...
  fBend128 = vesicle128.tracJump_upsample(X128,zeros(N,nv),1);
  ...
  farFieldtracJumpTrue = zeros(size(X));
  farFieldtracJumpForTenSolve = zeros(size(X));
  ...
  trueSelfBendSolve128 = zeros(128,nv);

  G = op.stokesSLmatrix(vesicle);
  G128 = op128.stokesSLmatrix(vesicle128);
  [~,Ten,Div] = vesicle.computeDerivs;
  [~,Ten128,Div128] = vesicle128.computeDerivs;
  for k = 1 : nv
    LHS = (Div(:,:,k)*G(:,:,k)*Ten(:,:,k));
    selfBend = G(:,:,k)*fBend(:,k);

    LHS128 = (Div128(:,:,k)*G128(:,:,k)*Ten128(:,:,k));
    selfBend128 = G128(:,:,k)*fBend128(:,k);
    
    RHS = -Div(:,:,k)*(vback(:,k)+farFieldtracJumpTrue(:,k)+selfBend);
    trueSelfBendSolve(:,k) = LHS\(Div(:,:,k)*selfBend);

    trueSelfBendSolve128(:,k) = LHS128\(Div128(:,:,k)*selfBend128);
  ...
  tension = tenTrue;
  % 
  vBackSolve = dnn.invTenMatOnVback32modes(X, vback + farFieldtracJumpTrue);
  % % 
  ...
  farFieldtracJumpForXsolve = zeros(size(X)); %farFieldtracJumpTrue;
  ...
  save(fname,'X', 'tracJumpBeforeTenSolve' , 'farFieldtracJumpForTenSolve', 'selfBendSolve','tension', 'trueVbackSolve','trueSelfBendSolve','trueSelfBendSolve128','farFieldtracJumpForXsolve', 'XadvTrue', 'XnewTrue')
