function submitDLDrunManyVes(flgSmooth,flgIntersect,flgSpacing,runName,theta,...
...
prams.vesViscCont = VCs; % viscosity contrast of the 1st type vesicle
prams.kappa = kappa; % bending rigidity
prams.volFrac = volFrac; % volume fraction in a DLD lane
prams.interpOrder = 5;
prams.variableKbDt = true;
prams.Nexact = 128;
errTol = 1e-2;
downTo = 16;
% save filename
...
dnn.repulsion = irepulsion; dnn.variableKbDt = prams.variableKbDt;
ttExact = dnn.ttExact;
%-------------------------------------------------------------------------
...
zzID = []; newID = [];
% putBackLimit = ceil((prams.periodN)/4)-1;
putBackLimit = 2;
...
for iter = 1 : 2
  ...
  [X,sig,etaInt,etaExt,RS] = ttExact.timeStepSimpleDiffDisc(X,sig,etaInt,etaExt,...
...
while time(end) < prams.Th
  ...
  [Xnew,signew,etaIntNew,etaExtNew,RSNew,vback] = dnn.DNNsolveMixed(...
  ...
  for k = 1 : prams.nv
    ...
    if ~isempty(xIntersect)
      ...
      for jts = 1 : 5
        ...
        vesicle.setUpRate();
        Xup = dnn.relaxExactSolve(vesicle, vup, prams.dt/5, Xup, ttExact.op);    
      end
  ...
  % JIGGLING
  Xhist = oc.fixCollisionsDLD(Xnew,XwallsInt);
  
  % AREA-LENGTH CORRECTION
  disp('Correcting area-length errors...')
  [Xnew2,ifail] = oc.correctAreaAndLength3(Xnew,area0,len0,16);
  ...
  [NearV2V,NearV2W] = vesicleUp.getZone(dnn.wallsInt,3);
  [icollisionVes,icollisionWall] = ...
    vesicleUp.collision(dnn.wallsInt,NearV2V,NearV2W,ttExact.fmm,ttExact.op);

  
  if icollisionVes || icollisionWall
    disp('there is a collision, so exact solve')
    ...
    vesicle.setUpRate();
    [Xnew,signew,etaIntNew,etaExtNew,RSNew,iter,iflag] = ttExact.timeStepSimpleDiffDisc(X,sig,etaIntStore,etaExtStore,RSstore,ones(nv,1),dnn.wallsInt,dnn.wallsExt,vesicle);
    if numel(Xhist(:,1))/2 ~= prams.Nexact 
  ...
  elseif max(errArea, errLen) < 0.9 * errTol
    ...
    Xhist = Xnew;
    sigStore = signew;
    etaIntStore = etaIntNew;
    etaExtStore = etaExtNew;
    RSstore = RSNew;
    ...
    area0,len0,nputLater,nZigZags,initCys,finalCys,iPartZZ,zzID,newID] = ...
      ...
      nZigZags,initCys,finalCys,zzID,newID);
...
function [Xhist,sigStore,Xfrozen,rem_ids,frozen_ids,prams,nPutBacks,...
    ...
    rem_ids,frozen_ids,nPutBacks,area0,len0,nputLater,nZigZags,initCys,finalCys,zzID,newID)
