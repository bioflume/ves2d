...
time_BIEM128 = time(1:nsteps);
nvesInCell_BIEM128 = nvesInCell(1:nsteps);
...
% nsteps = numel(time);
cx = mean(vesx(:,:,1:nsteps+1),1); cy = mean(vesy(:,:,1:nsteps+1),1);
cx = reshape(cx,nv,nsteps+1); cy = reshape(cy,nv,nsteps+1);

nvesInCell = zeros(nsteps+1,1);
vSize = 2.5;
...
time_VESNET32 = time(2:nsteps+1);
nvesInCell_VESNET32 = nvesInCell(2:nsteps+1);
