...
% dt = 1e-4, dts = 1e-4*2^(0:15), jj = 5, npar = 32
%%
clear;
kappa = 1;
% idx_dt = [8];
...
for iset = 1 : npar
    disp(['Set id: ' num2str(iset)])
    fileName = ['./relaxData6400/n256Dt' num2str(dtEffect) 'Relax100K_part' num2str(iset) '.mat'];
    load(fileName)
    nsampInSets(iset) = nInstances;
    Xstore = [Xstore XstandStore];
    XnewStore = [XnewStore XnewStandStore];
end
fileName = ['./relaxData6400/completeData/n256Dt' num2str(dtEffect) 'RelaxAllDataSet.mat'];
nInstances = sum(nsampInSets);
XstandStore = Xstore;
XnewStandStore = XnewStore;
dt = dtEffect;
