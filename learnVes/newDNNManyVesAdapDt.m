...
prams.Nexact = 32;
prams.nv = 10; %(24 for VF = 0.1, 47 for VF = 0.2) num. of vesicles
...
initialVesFile = './ICs/n10VesInCouette'; %'VF30_70VesIC';%'VF30_70VesIC'; % from another run % probVF20IC: fails in 2it
...
fileName = ['./output/Newnv' num2str(nv) 'N' num2str(N) 'n10.bin'];
%fileName = './output/failConfigResume.bin';
...
while time(end) < prams.Th
  ...
  [Xnew,signew,etanew,RSnew,vback] = dnn.newDNNLikeSolve(Xhist,sigStore,etaStore,RSstore,Nnet);
  % check if there is any self-intersecting shape
  for k = 1 : nv
    [xIntersect,~,~] = oc.selfintersect(Xnew(:,k));
    if ~isempty(xIntersect); disp('Self-intersection cannot be resolved'); end;
  end
  ...
  if (icollisionVes || icollisionWall); disp('There is a collision'); end;
