...
XpostFile = './OptimPost_RAp90_VFp5_Per6.mat'; % Xpost, Dpostx, Dposty, Dx, Dy are loaded
lenEll = 1.5;
volFrac = 0.05;
ireparam = true; % if false, then equal distribution along arc-length
time_adaptive = true; % if false, then fixed time step size
% Parameters
runName = 'ExpLong_OptimPost_VFp5';
putBackLimit = 6;
...
Nves = 48;
useFMM = true;
%saveFolder = '/workspace/gokberk/denseOptimRuns/RAp90_VFp5_Per6_OptimalLong/';
saveFolder = './output/';
prams.dtMax = 5e-3;
prams.dtMin = 1e-4;
prams.areaLenTol = 1e-3;
...
wbox = 36;
DLDscale = 0.05;
Ufar = (80 * 0.05 * 16)/Dy; % (80 is initial velocity, 0.05 DLD scale, 16 is initial Dy)
...
while curr_time < prams.Th
  ...
  [Xhist,sigStore,etaInt,etaExt,RS] = tt.timeStepSimpleDiffDisc(Xhist,sigStore,etaInt,etaExt,...
      RS,ones(prams.nv,1),dnn.wallsInt,dnn.wallsExt,vesicle);
  
  % CHECK IF SOLUTION IS ACCEPTIBLE
  if time_adaptive   
    [~,a,l] = oc.geomProp(Xhist);  
    [accept,dtScale,collUprate] = tt.newTimeStepSizeDerivs(Xhist,a,l,...
  ...
  if accept % if acceptable, then move on 
    ...
    if ireparam
      [Xiter, niter] = oc.reparametrize(Xhist,[],6,20);
    else
    ...
    [Xhist,sigStore,Xfrozen,rem_ids,frozen_ids,prams,nPutBacks,...
        ...
        nZigZags,initCys,finalCys,zzID,newID,tt,dnn.wallsInt, dnn.wallsExt);
  ...
  if rem(it,1) == 0
    save(fileName,'it','XhistStore','Xfrozen','initCys','finalCys','iPartZZ','time',...
...
function [Xhist,sigStore,Xfrozen,rem_ids,frozen_ids,prams,nPutBacks,...
    ...
    rem_ids,frozen_ids,nPutBacks,area0,len0,nputLater,nZigZags,initCys,finalCys,zzID,newID,tt,wallsInt,wallsExt)
...
if centxn>=prams.xfreeze
  ...
  vesIDsInlet = find(sqrt((centyS-centyNew).^2 + (centxS-centxNew).^2) <= 2*lenscale);
