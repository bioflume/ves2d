function [H1Norm] = LargeAnt_SemiLag_Solver(C_ic,vel,params,options,V1,V2)
% -------------------------------------------------------------------------
...
% ntime_smooth = ceil(1/deltaT_diff);
...
for tt = 1 : ntime
    ...
    Vx1 = V1(x,y,(tt-1)*deltaT_diff);
    Vx2 = V1(x,y,(tt-1)*deltaT_diff + deltaT_adv);
    Vx3 = V1(x,y,(tt-1)*deltaT_diff + 2*deltaT_adv);
    Vx4 = V1(x,y,(tt-1)*deltaT_diff + 3*deltaT_adv);
    
    Vy1 = V2(x,y,(tt-1)*deltaT_diff);
    Vy2 = V2(x,y,(tt-1)*deltaT_diff + deltaT_adv);
    Vy3 = V2(x,y,(tt-1)*deltaT_diff + 2*deltaT_adv);
    Vy4 = V2(x,y,(tt-1)*deltaT_diff + 3*deltaT_adv);

    vel1 = [Vx1(:);Vy1(:)]; vel2 = [Vx2(:);Vy2(:)]; vel3 = [Vx3(:);Vy3(:)]; vel4 = [Vx4(:);Vy4(:)];
